import { testTypes } from './../types/index';
    ...
    endTest() {
      ...
      const results = testWords.map((word) => {
        ...
        if (word.translations.includes(word.answer)) {
          ...
          if (word.level <= 90) {
            updates[`/users/${uid}/words/${word.id}/level`] = increment(10);
            wordData.changes = 10;
          } else {
            wordData.changes = 0;
          }
...
function composeTest(words: IWordData[], wordsCount: number, type: testTypes) {
  interface IWords {
    words: ITestWordData[];
  ...
  if (type === 'all') {
    ...
    return words
      ...
      .map((word) => ({ ...word, answer: '', isRight: null, changes: null }));
  }
}
